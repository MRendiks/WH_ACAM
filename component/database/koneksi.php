...
if (isset($_POST['daftarsesi'])) {
	...
	$sql = mysqli_query($koneksi, "SELECT * from pendaftaran where id_jadwal_dokter='$id_jadwal' AND id_pasien='$id_pasien'");
	while($data = mysqli_fetch_array($sql)){
		$id_pendaftaran = $data['id_pendaftaran'];
	}
	$query = mysqli_query($koneksi, "SELECT COUNT(*) totalantrian FROM pendaftaran WHERE id_jadwal_dokter='$id_jadwal'");
    $datantrian = mysqli_fetch_array($query);
    $data_antrian =$datantrian['totalantrian'];
	$insertantrian = mysqli_query($koneksi, "INSERT INTO antrian values('', '$id_pendaftaran', '$data_antrian')")or die ("Error in query: $insertantrian. ".mysqli_error($koneksi));
	if ($insertantrian) {
